    ...
    <section id="team" className="py-32 bg-black relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative">
        <div className="text-center mb-16 animate-section">
          ...
          <p className="text-gray-400 text-lg">대한민국 최고 수준의 개발 역량을 보유한 최고의 전문가들이 함께합니다</p>
        </div>
        <div className="grid md:grid-cols-3 lg:grid-cols-5 gap-8 max-w-7xl mx-auto">
          {teamMembers.map((member, index) => (
            <Card 
              key={index} 
              className={`animate-section team-card stagger-${index + 1} bg-gray-900/50 border-gray-800 p-6 hover:border-blue-500/50 transition-all text-center group ${index === 2 ? 'lg:col-start-3' : ''}`}
            >
